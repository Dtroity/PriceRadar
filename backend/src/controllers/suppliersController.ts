...
export async function patchSupplier(req: AuthRequest, res: Response) {
  ...
  const notify_channel =
    body.notify_channel == null ? undefined : String(body.notify_channel);
...
export async function addSupplierFilter(req: AuthRequest, res: Response) {
  const orgId = req.user?.organizationId;
  if (!orgId) return res.status(400).json({ error: 'Organization required' });
  const supplierId = req.params.id;
  const keyword = String((req.body as { keyword?: string } | undefined)?.keyword ?? '').trim();
  if (!keyword) return res.status(400).json({ error: 'keyword required' });
  const created = await filtersModel.createFilter({ organizationId: orgId, supplierId, keyword });
  return res.status(201).json(created);
}

export async function deleteSupplierFilter(req: AuthRequest, res: Response) {
  const orgId = req.user?.organizationId;
  if (!orgId) return res.status(400).json({ error: 'Organization required' });
  const supplierId = req.params.id;
...
export async function inviteSupplier(req: AuthRequest, res: Response) {
  const orgId = req.user?.organizationId;
  if (!orgId) return res.status(400).json({ error: 'Organization required' });
  const supplierId = req.params.id;
  ...
  if (!s.email) return res.status(400).json({ error: 'Supplier email is required' });
