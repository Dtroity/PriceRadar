...
router.use((req, res, next) => {
  const r = (req as AuthRequest).user?.role;
  if (r !== 'employee') return next();
...
router.delete('/suppliers/:id', requireRole(['super_admin', 'org_admin']), suppliersController.deleteSupplier);
...
router.get(
  '/upload/jobs/:jobId',
  requireModule('price_monitoring'),
  uploadJobController.getJobStatus
);

// Unified ingestion: classify → optional confirm → price list or document pipeline (сотрудники — только загрузка/журнал)
const ingestionRoles: UserRole[] = ['super_admin', 'org_admin', 'manager', 'employee'];
router.post(
  '/ingestion/init',
  requireRole(ingestionRoles),
  async (req, res, next) => {
    await ensureUploadDir();
    uploadMiddleware(req, res, (err) => {
      if (err) return res.status(400).json({ error: err.message });
...
router.post(
  '/ingestion/:id/confirm',
  requireRole(ingestionRoles),
  ingestionController.postConfirm
);
router.post(
  '/ingestion/:id/duplicate-decision',
  requireRole(ingestionRoles),
  ingestionController.postDuplicateDecision
);
