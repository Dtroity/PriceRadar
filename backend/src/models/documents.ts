...
export async function create(
  ...
  const { rows } = await pool.query(
    `INSERT INTO documents (organization_id, file_path, source_type, supplier_id, supplier_name, status)
     ...
     RETURNING id, organization_id, supplier_id, supplier_name, document_number, document_date, file_path, source_type, status, confidence, total_amount, created_at, updated_at`,
...
export async function getById(id: string, organizationId: string): Promise<Document | null> {
  const { rows } = await pool.query(
    `SELECT id, organization_id, supplier_id, supplier_name, document_number, document_date, file_path, source_type, status, confidence, total_amount, created_at, updated_at
...
export async function list(
  ...
  let query =
    'SELECT id, organization_id, supplier_id, supplier_name, document_number, document_date, file_path, source_type, status, confidence, total_amount, created_at, updated_at FROM documents WHERE organization_id = $1';
