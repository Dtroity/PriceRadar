 ...
 */
export async function compareAndSaveChanges(supplierId, newPriceListId, newRows, uploadDate) {
    const previousList = await priceListsModel.getPreviousPriceList(supplierId, uploadDate);
    if (!previousList)
        return { changes: [] };
    ...
    for (const row of newRows) {
        ...
        if (Math.abs(newPrice - oldPrice) < 0.01)
            continue;
        await priceChangesModel.createPriceChange(product.id, supplierId, oldPrice, newPrice, product.is_priority);
