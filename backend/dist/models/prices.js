...
export async function insertPrices(priceListId, items, options) {
    if (items.length === 0)
        return;
    ...
    for (const p of items) {
        if (oldPriceByProduct.has(p.product_id))
            continue;
        const { rows } = await pool.query(`SELECT p.price::text AS price
       FROM prices p
       ...
       LIMIT 1`, [p.product_id, pl.supplier_id, pl.organization_id]);
    ...
    await pool.query(`INSERT INTO prices (price_list_id, product_id, price, currency, document_id) VALUES ${parts.join(', ')}`, params);
    ...
    for (const p of items) {
        try {
            ...
            logger.warn({ err, priceListId, productId: p.product_id }, 'price_change audit insert failed (best-effort)');
        }
    }
    ...
    checkAndRecordAnomalies(anomalyItems)
        .then((n) => {
        if (n > 0)
            logger.info({ priceListId, anomalies: n }, 'price anomalies recorded');
    })
        .catch((err) => logger.warn({ err, priceListId }, 'anomaly detection failed'));
}
export async function getPricesByPriceListId(priceListId) {
    const { rows } = await pool.query('SELECT product_id, price, currency FROM prices WHERE price_list_id = $1', [priceListId]);
    return rows;
}
