...
export async function findProductByNormalizedName(normalizedName, organizationId) {
    if (organizationId) {
        const { rows } = await pool.query('SELECT id, name, normalized_name, is_priority, created_at FROM products WHERE organization_id = $1 AND normalized_name = $2', [organizationId, normalizedName]);
        return rows[0] ?? null;
    }
...
export async function createProduct(name, normalizedName, isPriority = false, organizationId) {
    if (organizationId) {
...
export async function findOrCreateProduct(name, normalizedName, isPriority = false, organizationId) {
 ...
 */
export async function mergeProducts(params) {
    ...
    try {
        ...
        const { rows: found } = await client.query(`SELECT id FROM products WHERE organization_id = $1 AND id = ANY($2::uuid[])`, [organizationId, allIds]);
       ...
       WHERE a.organization_id = b.organization_id
        ...
        await client.query(`UPDATE iiko_products_mapping SET product_id = $1::uuid WHERE product_id = ANY($2::uuid[])`, [tgt, src]);
        // Merge stock: add summed source quantities into target, then remove source rows
        const { rows: sumRows } = await client.query(`SELECT COALESCE(SUM(current_stock), 0) AS s
       FROM product_stock
       WHERE organization_id = $1::uuid AND product_id = ANY($2::uuid[])`, [organizationId, src]);
        const addQty = Number(sumRows[0]?.s ?? 0);
        const updStock = await client.query(`UPDATE product_stock
       SET current_stock = current_stock + $1::numeric, updated_at = NOW()
       WHERE organization_id = $2::uuid AND product_id = $3::uuid`, [addQty, organizationId, tgt]);
        if (updStock.rowCount === 0 && addQty !== 0) {
            const { rows: urow } = await client.query(`SELECT unit FROM product_stock
         WHERE organization_id = $1::uuid AND product_id = ANY($2::uuid[])
         ORDER BY updated_at DESC NULLS LAST
         LIMIT 1`, [organizationId, src]);
            await client.query(`INSERT INTO product_stock (organization_id, product_id, current_stock, unit)
         VALUES ($1::uuid, $2::uuid, $3::numeric, COALESCE($4, 'kg'))`, [organizationId, tgt, addQty, urow[0]?.unit ?? 'kg']);
        }
        await client.query(`DELETE FROM product_stock
       WHERE organization_id = $1::uuid AND product_id = ANY($2::uuid[])`, [organizationId, src]);
        // Same price list + product: keep one row
        await client.query(`DELETE FROM prices AS a
       USING prices AS b
       WHERE a.id > b.id
         AND a.price_list_id = b.price_list_id
         AND a.product_id = b.product_id
         AND a.product_id = $1::uuid`, [tgt]);
        await client.query(`DELETE FROM supplier_prices_history AS a
       ...
       WHERE a.id > b.id
        ...
        await client.query(`WITH dup AS (
         SELECT recipe_id,
                product_id,
       ...
       FROM dup
       WHERE ri.id = dup.keep_id`, [tgt]);
        await client.query(`DELETE FROM recipe_items AS ri
       ...
       SET quantity = dup.total_q
       FROM dup
       WHERE oi.id = dup.keep_id`, [tgt]);
        await client.query(`DELETE FROM supplier_order_items AS oi
       ...
       WHERE a.organization_id = b.organization_id
        ...
        await client.query(`INSERT INTO product_audit_log (organization_id, product_id, action, actor_id, meta)
