/**
 * Unified ingestion pipeline (ingestion_jobs + rollback link on price_changes).
...
exports.up = async function up(knex) {
  ...
  `);
  await knex.raw(`
    CREATE INDEX IF NOT EXISTS idx_ingestion_org_created
      ON ingestion_jobs(organization_id, created_at DESC);
  `);
  await knex.raw(`
    CREATE INDEX IF NOT EXISTS idx_ingestion_org_hash
      ON ingestion_jobs(organization_id, file_sha256);
  `);
  await knex.raw(`
    ALTER TABLE price_changes
      ADD COLUMN IF NOT EXISTS source_price_list_id UUID REFERENCES price_lists(id) ON DELETE CASCADE;
  `);
};
...
exports.down = async function down(knex) {
  await knex.raw(`
    ALTER TABLE price_changes
      DROP COLUMN IF EXISTS source_price_list_id;
  `);
  await knex.raw('DROP TABLE IF EXISTS ingestion_jobs');
};
