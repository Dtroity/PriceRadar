...
/**
 * Журнал единого конвейера загрузок (прайсы + документы): статус, откат, ссылка на распознанный документ.
 */
export default function IngestionJournalSection({ isEmployee, refreshKey = 0 }: Props) {
  const t = useT();
  ...
  return (
    <section id="ingestion-journal" className="scroll-mt-4 space-y-3">
      <div>
        <h2 className="text-lg font-semibold text-slate-800">{t('documents.ingestionJournalTitle')}</h2>
        <p className="mt-1 text-sm text-slate-600">{t('documents.ingestionJournalLead')}</p>
      </div>
